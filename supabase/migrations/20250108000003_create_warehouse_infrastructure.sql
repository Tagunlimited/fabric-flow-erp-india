-- Create missing warehouse infrastructure
...
CREATE POLICY "Enable insert for authenticated users only" ON warehouses FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Enable update for authenticated users only" ON warehouses FOR UPDATE USING (auth.role() = 'authenticated');
CREATE POLICY "Enable delete for authenticated users only" ON warehouses FOR DELETE USING (auth.role() = 'authenticated');

CREATE POLICY "Enable read access for all users" ON warehouse_floors FOR SELECT USING (true);
CREATE POLICY "Enable insert for authenticated users only" ON warehouse_floors FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Enable update for authenticated users only" ON warehouse_floors FOR UPDATE USING (auth.role() = 'authenticated');
CREATE POLICY "Enable delete for authenticated users only" ON warehouse_floors FOR DELETE USING (auth.role() = 'authenticated');

CREATE POLICY "Enable read access for all users" ON warehouse_racks FOR SELECT USING (true);
CREATE POLICY "Enable insert for authenticated users only" ON warehouse_racks FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Enable update for authenticated users only" ON warehouse_racks FOR UPDATE USING (auth.role() = 'authenticated');
CREATE POLICY "Enable delete for authenticated users only" ON warehouse_racks FOR DELETE USING (auth.role() = 'authenticated');

CREATE POLICY "Enable read access for all users" ON warehouse_bins FOR SELECT USING (true);
CREATE POLICY "Enable insert for authenticated users only" ON warehouse_bins FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Enable update for authenticated users only" ON warehouse_bins FOR UPDATE USING (auth.role() = 'authenticated');
CREATE POLICY "Enable delete for authenticated users only" ON warehouse_bins FOR DELETE USING (auth.role() = 'authenticated');
...
-- 8. Create default warehouse structure
-- First check if warehouses table exists and what columns it has
DO $$
BEGIN
    ...
    ELSE
        -- Create table with all columns first
        ...
        -- Then insert with all columns
        INSERT INTO warehouses (id, name, code, address, city, state, pincode, contact_person, phone, email) 
        VALUES (
            '00000000-0000-0000-0000-000000000001',
            'Main Warehouse',
            'MAIN-WH',
...
CREATE TRIGGER handle_updated_at_warehouses BEFORE UPDATE ON warehouses
FOR EACH ROW EXECUTE FUNCTION moddatetime('updated_at');

CREATE TRIGGER handle_updated_at_warehouse_floors BEFORE UPDATE ON warehouse_floors
FOR EACH ROW EXECUTE FUNCTION moddatetime('updated_at');

CREATE TRIGGER handle_updated_at_warehouse_racks BEFORE UPDATE ON warehouse_racks
FOR EACH ROW EXECUTE FUNCTION moddatetime('updated_at');

CREATE TRIGGER handle_updated_at_warehouse_bins BEFORE UPDATE ON warehouse_bins
FOR EACH ROW EXECUTE FUNCTION moddatetime('updated_at');
