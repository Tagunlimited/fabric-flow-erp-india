-- Initialize fabric_inventory rows for an order's fabrics if none exist
...
begin
  ...
  for r in (
    select oi.fabric_id, sum(coalesce(oi.quantity,0))::numeric as qty
    ...
    -- If no inventory row exists for this fabric, create one in MAIN with available = order qty
    if not exists (select 1 from public.fabric_inventory fi where fi.fabric_id = r.fabric_id) then
      ...
      values (r.fabric_id, v_zone_id, coalesce(r.qty,0), 0, coalesce(p_default_unit,'meters'));
...
select 'ensure_fabric_inventory_for_order created' as status;
