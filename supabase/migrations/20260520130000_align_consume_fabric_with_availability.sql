-- Cutting UI counts variant-matched GRN rows; consume_fabric_for_cutting must use the same rules.
  ...
  SELECT EXISTS (
    ...
    WHERE fm.id = p_fabric_id
      ...
      AND (
        ...
        OR (
          lower(trim(COALESCE(gi.fabric_name, poi.fabric_name, split_part(p_wi.item_name, ' - ', 1), p_wi.item_name, '')))
...
BEGIN
  ...
  -- Link orphan GRN rows to fabric_master before availability check
  ...
  WHERE wi.grn_item_id = gi.id
    ...
    AND (
      poi.fabric_id = p_fabric_id
      OR public.warehouse_row_matches_fabric_for_cutting(p_fabric_id, wi)
    );
...
GRANT EXECUTE ON FUNCTION public.warehouse_row_matches_fabric_for_cutting(uuid, public.warehouse_inventory) TO authenticated;
