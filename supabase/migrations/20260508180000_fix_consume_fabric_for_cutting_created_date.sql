...
-- ISSUE
-- consume_fabric_for_cutting() orders by wi.created_date, but warehouse_inventory
-- does not have created_date in current schema (uses created_at/updated_at).
--
-- RESOLUTION
-- Replace function and remove wi.created_date from ORDER BY.
...
BEGIN
  ...
  -- Align with warehouse inventory scope:
  -- IN_STORAGE + STORAGE bins OR READY_TO_DISPATCH + DISPATCH_ZONE (legacy).
  -- Match fabric via item_id OR PO line fabric_id when GRN row has no item_id.
  -- Sum allocations from bom_records (not legacy bom_items).
  ...
  WHERE wi.item_type = 'FABRIC'
    AND (
      wi.item_id = p_fabric_id
      OR EXISTS (
        SELECT 1
        FROM public.grn_items gi
        INNER JOIN public.purchase_order_items poi ON poi.id = gi.po_item_id
        WHERE gi.id = wi.grn_item_id
          AND poi.fabric_id = p_fabric_id
      )
    )
    AND (
