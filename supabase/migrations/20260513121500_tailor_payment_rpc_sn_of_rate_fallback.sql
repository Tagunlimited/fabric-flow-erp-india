-- SN/OF amounts were ₹0 when rates existed only on order_items (no line tag in notes)
-- or when order_assignments join was ambiguous / missing. Resolve rates from:
--   1) order_items row matching [line:uuid] in notes
--   2) latest order_assignments row for the order (LATERAL LIMIT 1)
--   3) best-effort order_items row on the same order (prefer rows with any non-null rate)
...
COMMENT ON FUNCTION public.tailor_payment_batch_summary(date, date, timestamptz, timestamptz) IS
  'Batch-level tailor payment metrics; SN/OF rates from tagged line, else order_assignments, else any order_items row on the order.';
