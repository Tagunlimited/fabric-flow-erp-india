-- Add fabric_for_supplier to pending_po_items_view
-- This allows displaying supplier-specific fabric names in the Purchase Order Planning page
...
SELECT
  ...
  bri.fabric_name,
  -- Get fabric_for_supplier by matching fabric_name, color, and gsm
  (SELECT fabric_for_supplier FROM fabric_master 
   WHERE fabric_name = bri.fabric_name 
     AND (bri.fabric_color IS NULL OR color = bri.fabric_color)
     AND (bri.fabric_gsm IS NULL OR gsm = bri.fabric_gsm)
   LIMIT 1) AS fabric_for_supplier,
  bri.fabric_color,
  bri.fabric_gsm,
  bri.item_id,
  bri.item_code,
