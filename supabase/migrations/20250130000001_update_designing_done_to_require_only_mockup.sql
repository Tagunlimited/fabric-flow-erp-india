-- Update recalc_order_status function to only require mockup_images (not reference_images)
-- The status should change to 'designing_done' when mockup is uploaded, regardless of reference image
...
BEGIN
  ...
  -- Handle readymade orders separately (they don't need design images)
  ...
  -- Design images check - ONLY check for mockup_images (not reference_images)
  SELECT COUNT(*)
    INTO v_item_count
  FROM public.order_items oi
  WHERE oi.order_id = p_order_id;

  -- UPDATED: Only check for mockup_images, not reference_images
  SELECT COUNT(*)
    INTO v_done_count
  FROM public.order_items oi
  WHERE oi.order_id = p_order_id
  ...
  -- 1. When mockup images are uploaded, set to designing_done (base design stage)
  -- UPDATED: Only requires mockup_images, not reference_images
...
COMMENT ON FUNCTION public.recalc_order_status(uuid) IS 'Recomputes orders.status using receipts, design uploads (mockup only), and BOM presence. Status changes to designing_done when mockup is uploaded, regardless of reference image.';
