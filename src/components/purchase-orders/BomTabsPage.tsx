...
import { Eye, Search, Package, FileText } from 'lucide-react';
  ...
  const fetchOrdersWithoutBom = async () => {
    try {
      ...
      const eligibleOrders = allOrders.filter((order) => {
        ...
        if (isOrderStatusCompleted(order)) return false;
  ...
  const fetchOrdersCompleteBom = async () => {
    try {
      ...
      const eligible = allOrders.filter((order) => {
        const itemCount = (order.order_items || []).length;
        if (itemCount === 0) return false;
        return orderFullyCoveredByBom(order, bomList) || isOrderStatusCompleted(order);
      });
      ...
      <div className="grid grid-cols-1 gap-4">
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Complete / full BOM</p>
                <p className="text-2xl font-bold">{filteredOrders.length}</p>
              </div>
              <div className="h-8 w-8 bg-emerald-100 rounded-full flex items-center justify-center">
                <span className="text-emerald-700 text-sm font-bold">{filteredOrders.length}</span>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            ...
            a BOM, or the order is marked completed.
          </p>
        </div>
