...
import { ShoppingCart, Eye, Package, Clock, CheckCircle, Filter, FileImage, X } from "lucide-react";
...
const DesignDesignsPage = () => {
  ...
  const pendingOrders = orders.filter(order => !isDesignWorkComplete(order));
  const completedOrders = orders.filter(order => isDesignWorkComplete(order));
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Orders with receipts — mockups and branding (Design &amp; Printing)
          </p>
        </div>
...
export default DesignDesignsPage;
