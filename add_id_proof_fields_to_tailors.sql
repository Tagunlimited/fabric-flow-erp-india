-- ============================================================================
-- ADD ID PROOF FIELDS TO TAILORS TABLE
-- Generated: January 21, 2025
-- Description: Adds ID proof image and number fields to tailors table
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_tailors_id_proof_number ON tailors(id_proof_number);

-- Add comment to document the new fields
COMMENT ON COLUMN tailors.id_proof_type IS 'Type of ID proof document (Aadhaar, PAN, etc.)';
COMMENT ON COLUMN tailors.id_proof_number IS 'ID proof document number';
COMMENT ON COLUMN tailors.id_proof_image_url IS 'URL of the front image of ID proof document';
COMMENT ON COLUMN tailors.id_proof_back_image_url IS 'URL of the back image of ID proof document (if applicable)';

-- Update the check constraint to make ID proof fields required for new tailors
-- Note: This will only apply to new records, existing records can be updated separately
ALTER TABLE tailors 
ADD CONSTRAINT check_tailor_id_proof_required 
CHECK (
...
-- Create a function to validate ID proof numbers based on type (reuse from employees)
CREATE OR REPLACE FUNCTION validate_tailor_id_proof_number(proof_type TEXT, proof_number TEXT)
RETURNS BOOLEAN AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION validate_tailor_id_proof()
RETURNS TRIGGER AS $$
BEGIN
  -- Only validate if ID proof fields are provided
  IF NEW.id_proof_type IS NOT NULL AND NEW.id_proof_number IS NOT NULL THEN
    IF NOT validate_tailor_id_proof_number(NEW.id_proof_type, NEW.id_proof_number) THEN
...
-- Create the trigger
DROP TRIGGER IF EXISTS trigger_validate_tailor_id_proof ON tailors;
CREATE TRIGGER trigger_validate_tailor_id_proof
  BEFORE INSERT OR UPDATE ON tailors
  FOR EACH ROW
  EXECUTE FUNCTION validate_tailor_id_proof();
...
-- Verification query
SELECT 
  'ID proof fields added to tailors table successfully!' as status,
