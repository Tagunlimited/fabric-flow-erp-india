-- Warehouse Master Database Schema - Fixed Version
-- Handles existing location_type enum

-- Drop existing location_type enum if it exists and recreate with new values
...
CREATE INDEX IF NOT EXISTS idx_racks_code ON racks(rack_code);
...
DROP POLICY IF EXISTS "Authenticated users can view all floors" ON floors;
DROP POLICY IF EXISTS "Authenticated users can manage floors" ON floors;
DROP POLICY IF EXISTS "Authenticated users can view all racks" ON racks;
DROP POLICY IF EXISTS "Authenticated users can manage racks" ON racks;
DROP POLICY IF EXISTS "Authenticated users can view all bins" ON bins;
DROP POLICY IF EXISTS "Authenticated users can manage bins" ON bins;
